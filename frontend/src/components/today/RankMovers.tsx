...
export default function RankMovers({ movers, matchHeightPx }: RankMoversProps) {
  ...
  const filtered = useMemo(
    () => (selectedTeams.length === 0 ? movers : movers.filter(m => selectedTeams.includes(m.team_name))),
    [movers, selectedTeams],
  )
  ...
  return (
    <div
      ...
      className="flex min-h-0 flex-col rounded-lg bg-white p-4 shadow dark:bg-gray-800"
    >
      <div className="flex flex-wrap items-baseline justify-between gap-2 pb-2">
        ...
        <div className="flex items-center gap-2">
          ...
          {movers.length > 0 && (
            <TeamFilterDropdown teams={teamOptions} selected={selectedTeams} onChange={setSelectedTeams} />
          )}
        </div>
      </div>
      ...
      {movers.length === 0 ? (
        ...
        <p className="py-3 text-xs text-gray-500 dark:text-gray-400">No movers for the selected teams.</p>
      ) : (
        <div className="max-h-[60vh] min-h-0 flex-1 overflow-x-auto overflow-y-auto" style={matchHeightPx ? { maxHeight: 'none' } : undefined}>
