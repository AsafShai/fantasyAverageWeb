...
function getProbColor(prob: number): { bg: string; text: string } {
  if (prob === 0) return { bg: 'rgb(240,245,255)', text: '#aaa' }
    ...
    <div>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full border-collapse text-sm">
          <thead>
            <tr>
              <th className="px-3 py-2 text-left font-medium text-gray-700 whitespace-nowrap">Team</th>
              {rankCols.map(r => (
                <th key={r} className="px-2 py-2 text-center font-medium text-gray-700 w-12">{r}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {sorted.map(team => {
              ...
              return (
                <tr
                  key={team.team_id}
                  className={`transition-all duration-150 ${isSelected ? 'border-[3px] border-black' : ''}`}
                >
                  <td
                    className={`px-3 py-1.5 whitespace-nowrap cursor-pointer transition-colors duration-150 ${isSelected ? 'font-bold text-gray-900' : 'font-medium text-gray-800 hover:bg-gray-100'}`}
                  ...
                  {rankCols.map(r => {
                    ...
                    const { bg, text } = getProbColor(prob)
                    return (
                      <td
                        key={r}
...
function RankingTable({ rankings }: { rankings: TeamRanking[] }) {
  ...
  const thClass = (key: RankingSortKey) =>
    `px-2 py-2 text-center font-medium text-xs cursor-pointer select-none whitespace-nowrap hover:text-blue-600 ${sortKey === key ? 'text-blue-600' : 'text-gray-700'}`
        ...
        <table className="min-w-full text-sm">
          <thead>
            <tr className="border-b border-gray-200">
              <th onClick={() => handleSort('rank')} className={`px-3 py-2 text-left font-medium text-xs cursor-pointer select-none hover:text-blue-600 ${sortKey === 'rank' ? 'text-blue-600' : 'text-gray-700'}`}>#{ arrow('rank')}</th>
              <th className="px-3 py-2 text-left font-medium text-gray-700 text-xs">Team</th>
              {STAT_KEYS.map(k => (
                <th key={k} onClick={() => handleSort(`expected_pts_${k}`)} className={thClass(`expected_pts_${k}`)}>
                  {STAT_LABELS[k]}{arrow(`expected_pts_${k}`)}
                </th>
              ))}
              <th onClick={() => handleSort('total_expected_pts')} className={`${thClass('total_expected_pts')} border-l-2 border-gray-300`}>
          ...
          <tbody>
            {sorted.map(team => (
              <tr key={team.team_id} className="border-b border-gray-100 hover:bg-gray-50">
                ...
                <td className="px-3 py-2 text-center font-semibold border-l-2 border-gray-300">
...
function ProjectionSummary({ predictions }: { predictions: TeamPrediction[] }) {
  ...
  const thClass = (key: ProjectionSortKey) =>
    `px-3 py-2 text-center font-medium text-xs cursor-pointer select-none whitespace-nowrap hover:text-blue-600 ${sortKey === key ? 'text-blue-600' : 'text-gray-700'}`
    ...
    <div>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full text-sm">
          <thead>
            <tr className="border-b border-gray-200">
              <th onClick={() => handleSort('team_name')} className={`px-3 py-2 text-left font-medium text-xs cursor-pointer select-none hover:text-blue-600 ${sortKey === 'team_name' ? 'text-blue-600' : 'text-gray-700'}`}>
                ...
                {STAT_KEYS.map(k => {
                  ...
                  return (
                    <td key={k} className="px-3 py-2 text-center text-xs">
                      ...
                      <span className="text-gray-400 ml-0.5">+/-{formatStat(k, stddev)}</span>
                    </td>
                  )
                })}
                <td className="px-3 py-2 text-center text-xs text-gray-500">
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8 overflow-x-hidden">
      <div className="bg-white rounded-lg shadow p-4 sm:p-6 overflow-hidden">
        <h1 className="text-2xl font-bold text-gray-900 mb-6">Season Estimator</h1>
