...
describe('MockProjectedStandings', () => {
  ...
  it('renders a heatmap table and standings controls', async () => {
    ...
    expect(screen.queryByRole('checkbox', { name: 'Show GP' })).toBeNull()
    await user.click(screen.getByRole('button', { name: 'Totals' }))
    expect(screen.getByRole('checkbox', { name: 'Show GP' })).toBeChecked()
    expect(screen.getByRole('columnheader', { name: 'GP' })).toBeInTheDocument()
    await user.click(screen.getByRole('checkbox', { name: 'Show GP' }))
    expect(screen.queryByRole('columnheader', { name: 'GP' })).toBeNull()
    await user.click(screen.getByRole('checkbox', { name: 'Show GP' }))
    expect(screen.getByRole('columnheader', { name: 'GP' })).toBeInTheDocument()
