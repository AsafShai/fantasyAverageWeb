    ...
    try:
        async with pool.acquire() as conn:
            row = await conn.fetchrow(
                "SELECT payload, source, fetched_at FROM adp_provider_cache WHERE provider = $1",
                provider,
            )
    except Exception:
...
async def _save_to_db(entry: ProviderCacheEntry) -> None:
    ...
    try:
        async with pool.acquire() as conn:
            await conn.execute(
                """
                INSERT INTO adp_provider_cache (provider, payload, source, fetched_at)
                VALUES ($1, $2, $3, $4)
                ON CONFLICT (provider) DO UPDATE
                SET payload = EXCLUDED.payload, source = EXCLUDED.source, fetched_at = EXCLUDED.fetched_at
                """,
                entry.provider,
                entry.payload,
                entry.source,
